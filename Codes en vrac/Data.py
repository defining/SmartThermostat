...
class ThermoMeasureHandler(QueryHandler):
    ...
    def get_last_presence(self, device_name, max_time=24*3600):
        """
    ...
    def get_presence_by_day_and_by_time_interval(self, day, time_min, time_max, n_limit=150, date_limit=2000000000):
        """
        ...
        WHERE = "strftime('%w', date(date, 'unixepoch'))== '" + str(day) + "' AND \
         ...
         presence != 'NULL' AND date < " + str(date_limit)

        return self.select(COLS, FROM, WHERE, n_limit)


    def get_relevant_times_of_today(self, today, n_limit=-1, date_limit=2000000000):
        """
