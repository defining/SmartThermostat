...
#define NETWORK_SSID   "TRANH201-G11"  // DONE Todo: adapt to your group's ssid
...
#define THERMISTOR_ANALOG_PIN   A4      // DONE Todo: adapt the pins to your setup
...
{
  // DONE Todo: Complete the code to return the currently sensed temperature.

  float ref_resistance = 10000.0;
  
  //01. Reading of the analog tension from the thermistor
  int sensorVal = analogRead(THERMISTOR_ANALOG_PIN);

  //02. Conversion from bits to voltage in V
  float voltage = (sensorVal/1024.0)*4.64;
  
  //03. Calculation of the thermistor resistance from the voltage read and the reference resistance.
  float resistance_thermistor = (ref_resistance*voltage)/(4.64-voltage);

  //04. Calculation of the temperature (Kelvin) using Steinhart-Hart relation 
  float a = 3.354016E-3;
  float b = 2.569850E-4;
  float c = 2.620131E-6;
  float d = 6.383091E-8;

  float temp_kelvin = pow(a + b*log(resistance_thermistor/ref_resistance) + c*pow(log(resistance_thermistor/ref_resistance),2)
  + d* pow(log(resistance_thermistor/ref_resistance),3),-1);

  //05. Conversion into Celsius
...
{
  // DONE Todo: Complete the code to return the sensed presence.
  return digitalRead(PIR_DIGITAL_PIN);
}
...
int readValve()
{
  // DONE Todo: Complete the code to return the current valve setting (0-100).
  return (int)analogRead(VALVE_ANALOG_PIN)*100.0/1024.0;
}

int setValve(int value)
{
  // DONE Todo: Complete the code to set the current valve setting (0-100).
  digitalWrite(VALVE_ANALOG_PIN, float(value)/100.0*1024.0);
}
...
boolean serveRawContent(const String& content, int cip=0) 
{
  // FixMe: We hardcode the channel to 0 here, but the one returned
  //        from the server (after the "+IPD:") should be used instead.
  return    espCmd( "AT+CIPSEND=" + String(cip) + "," + String(content.length()) , "" )
         && espCmd( 200, content ); //was 200
}
...
void serveHttpContent(const String& ContentType, const String& Content, int cip=0) 
{  
...
{
  // DONE Todo: Adapt to serve the current temperature reading according to the 
  //       communication protocol.

  serveHttpContent("plain/text", String(readThermistor(), 3)); // float -> string
}

void servePresence()
{
  // DONE Todo: Adapt to serve the current presence reading according to the 
  //       communication protocol.
  //dbg.print("serve");
  serveHttpContent("plain/text", String(readPirState()));
}

void serveValveGet()
{
  // DONE Todo: Adapt to serve the current valve setting according to the 
  //       communication protocol.

  serveHttpContent("plain/text", String(readValve()));
}
